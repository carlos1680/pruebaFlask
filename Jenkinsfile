		...
		stage('Actualizar contenedor'){
			...
			steps {
				sshagent(credentials: ['MiVpsLogin']) {
				  sh "ssh  -o StrictHostKeyChecking=no -tt root@server.mechabios.com 'docker-compose -f :/var/data/infraestructura/docker/fuentes/testing/ docker-compose-testing.yaml up -d --force-recreate --build flesk'"
				}
			}
		}
	}
